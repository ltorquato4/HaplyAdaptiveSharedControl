...
def calculate_metrics(df):
    """Calculates orthogonal error and normalized distance[cite: 2]."""
    ...
    # Absolute orthogonal error (magnitude of deviation)[cite: 2]
    ...
    # Position along the reference trajectory[cite: 2]
...
def plot_directories_error_by_mode(base_data_dir="../processed_logs", output_dir="../plots/comparison_plots"):
    """Plots mean positional errors separated by mode and phase across all directories[cite: 2]."""
    ...
    # Extract and calculate data for all runs[cite: 2]
        ...
        for file in csv_files:
            try:
                ...
                for mode, mode_df in df.groupby('study_controller_mode'):
                    ...
                    # 2. Store INDIVIDUAL PHASE data for this file[cite: 2]
    ...
    # Generate the plots[cite: 2]
    for mode, phases_dict in mode_data.items():
        # Get individual phases and append 'all_phases' at the bottom[cite: 2]
        ...
        for i, phase in enumerate(plot_phases):
            ...
            for run_name, errors in phases_dict[phase].items():
                if errors:
                    ...
                    ax.plot(common_norm_dist, mean_err, linewidth=2, label=f"Run: {run_name}")
                    plotted_any = True
            
            # Label the subplot with the phase name inline[cite: 2]
            ...
            # Place legend only on the top subplot
            if i == 0 and plotted_any:
                ax.legend(loc='upper right', fontsize=9)
                
        # Global limits and formatting[cite: 2]
