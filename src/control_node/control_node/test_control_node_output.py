...
class ControlOutputVisualizer(Node):
    """Display the current two-dimensional controller output."""

    def __init__(self):
        """Create the controller-output subscription."""
        ...
        self.get_logger().info(
            "Visualizer Node initialized and listening on /control/U_a"
        )

    def control_callback(self, msg: Vector3):
        """Store the most recent controller output."""
        self.latest_control_x = msg.x
        self.latest_control_y = msg.y


def main(args=None):
    """Run the controller-output visualizer."""
    rclpy.init(args=args)
    node = ControlOutputVisualizer()
    ...
    try:
        while rclpy.ok():
            ...
            end_sum = (
                center[0] + current_x * scale,
                center[1] - current_y * scale,
            )

            draw_arrow(screen, (255, 50, 50), center, end_x)
            draw_arrow(screen, (50, 255, 50), center, end_y)
            draw_arrow(screen, (100, 150, 255), center, end_sum)
            ...
            text_sum = font.render(
                f"Sum Mag: {magnitude:.2f}",
                True,
                (100, 150, 255),
            )
