...
The repository-level `scripts/run_experiment.py` questionnaire wrapper adds
`questionnaire/questionnaire.csv` below the matching session directory. The
nested location keeps the responses with the participant data without adding a
non-trial CSV to the files scanned by `study_analysis`.
